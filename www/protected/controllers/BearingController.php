...
class BearingController extends Controller
{
	...
	{
		$model=new Bearing;
		$parent = MachineNode::model()->findByPk($parent);
		if (!$parent) {
		...
		{
			$model->attributes=$_POST['Bearing'];
	...
	{
		$model=Bearing::model()->findByAttributes(array('tree_id'=>(int)$tree_id));
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='bearing-form')
		{
